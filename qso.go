...
type QsoDetails struct {
	...
	Freq        string `json:"freq" validate:"freq"`
	...
	QsoDate     string `json:"qso_date" validate:"qso_date"`
	QsoDateOff  string `json:"qso_date_off" validate:"qso_date_off"`
	QsoRandom   string `json:"qso_random"`
	QsoComplete string `json:"qso_complete"`
	RstRcvd     string `json:"rst_rcvd" validate:"rst_rcvd"`
	RstSent     string `json:"rst_sent" validate:"rst_sent"`
	...
	TimeOff     string `json:"time_off" validate:"time_off"`
	TimeOn      string `json:"time_on" validate:"time_on"`
...
type ContactedStation struct {
	...
	Call           string `json:"call" validate:"required,uppercase,min=3,max=12"`
	...
	RequiresUpdate bool   `json:"requires_update"` // Not part of ADI, indicates if this contacted station's data needs to be refreshed
}
...
type LoggingStation struct {
	...
	StationCallsign string `json:"station_callsign" validate:"required,uppercase,min=3,max=12"`
}
